...
node[:vim][:users].each do |user|
  # NeoBundleを入れるディレクトリの作成
  execute "make bundle directory for vim" do
    not_if {File.exists?("#{user[:dir]}/.vim/bundle")}
    command "mkdir -p #{user[:dir]}/.vim/bundle"
    user "#{user[:name]}"
    group "#{user[:name]}"
    action :run
  end
  ...
  # NeoBundleをインストール
  git "#{user[:dir]}/.vim/bundle/neobundle.vim" do
